...
Tauri 2 + чистый HTML/CSS/JS. Без Svelte, без TypeScript, без Vite/Deno-сборки —
фронтенд отдаётся как есть из `src/`, сборщик не нужен.
...
## Не реализовано (не входило в текущий объём)

- Экраны глав/заданий (были в предыдущей Svelte-версии, в этой — только логин + карточка пользователя, как просили)
- UI редактирования глав/заданий для editor/admin
- UI управления лицензиями для admin
- Настоящие иконки — сейчас плейсхолдер (`icons/`), без `icon.icns`:
  ```bash
  cargo tauri icon path/to/logo.png
  ```
- Сборка не проверялась вживую (нет сети в среде, где это писалось) — при первом `cargo tauri dev` возможны версийные нестыковки, поправь по сообщениям компилятора
